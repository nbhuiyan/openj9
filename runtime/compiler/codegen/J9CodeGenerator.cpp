   ...
   if (node->getOpCode().isCall() &&
      ...
      if(rm == TR::java_lang_invoke_MethodHandle_invokeBasic ||
        ...
        rm == TR::java_lang_invoke_MethodHandle_linkToInterface)
         ...
         // If the linkToStatic call is not for an unresolved invokedynamic/invokehandle, then the JIT would not create a push
         // of a null appendix object, so the VM would not need to do any stack adjustments. In those cases, -1 is stored.
         // This is also done for linkToSpecial, as it shares the same handling mechanism in the VM.
