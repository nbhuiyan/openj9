...
; Note that the double is assumed to have an exponent >= 32.
        ...
        align 16
SSEdouble2LongIA32:
        mov edx, dword [_rsp+8]      ; load the double into edx:eax
        mov eax, dword [_rsp+4]
...
%ifdef TR_HOST_64BIT
        mov   _rcx, qword [_rbp + eq_J9VMThread_heapAlloc]
%else
        mov   _rcx, dword [_rbp + eq_J9VMThread_heapAlloc]
%endif
...
%ifdef TR_HOST_64BIT
        mov   qword [_rbp + eq_J9VMThread_PrefetchCursor], 384
%else
        mov   dword [_rbp + eq_J9VMThread_PrefetchCursor], 384
%endif
prefetch_done:
        pop   _rcx
        ret
...
%ifdef TR_HOST_64BIT
        mov         _rdx, qword [_rbp + eq_J9VMThread_heapAlloc]       ; _rdx = O+S
        mov         ecx, dword [_rbp + eq_J9VMThread_PrefetchCursor]  ; _rcx = current trigger boundary (B)
%else
        mov         _rdx, dword [_rbp + eq_J9VMThread_heapAlloc]       ; _rdx = O+S
        mov         _rcx, dword [_rbp + eq_J9VMThread_PrefetchCursor]  ; _rcx = current trigger boundary (B)
%endif
...
%ifdef TR_HOST_64BIT
        mov         _rdx, qword [_rbp + eq_J9VMThread_heapTop]
%else
        mov         _rdx, dword [_rbp + eq_J9VMThread_heapTop]
%endif
...
%else
        mov         dword [_rbp + eq_J9VMThread_PrefetchCursor], _rcx
%endif
        pop         _rsi
        pop         _rdx
        pop         _rcx
