...
#define DEFAULT_FORCEINLINE_MULTIPLIER 2

#define DEFAULT_MAX_ANALYZED_SIZE_THRESHOLD 500000
...
// VarHandle object through folding ValueLayouts$AbstractValueLayout.accessHandle() if the VarHandle has been created
   ...
   int32_t analyzedSizeThreshold = callerAnalyzedSizeThreshold ? callerAnalyzedSizeThreshold : allowanceFactor * sizeThreshold;

   bool sizeResetRequiredForForceInline = false;
   ...
   if (_inliner->alwaysWorthInlining(calltarget->_calleeMethod, NULL))
      ...
      static int32_t maxAnalyzedSizeThreshold = DEFAULT_MAX_ANALYZED_SIZE_THRESHOLD;
      static const char *forceInlineMultiplierStr = feGetEnv("TR_ForceInlineMultiplier");
      static const char *maxAnalyzedSizeThresholdStr = feGetEnv("TR_MaxAnalyzedSizeThreshold");
      if (forceInlineMultiplierStr) forceInlineMultiplier = atoi(forceInlineMultiplierStr);
      if (maxAnalyzedSizeThresholdStr) maxAnalyzedSizeThreshold = atoi(maxAnalyzedSizeThresholdStr);

      // Before increasing analyzedSize threshold, we should check if the the analyzedSizeThreshold has
      // already exceeded the limit maxAnalyzedSizeThreshold, at which point we should stop increasing
      // the analyzed size threshold any further. maxAnalyzedSizeThreshold can be modified via env
      // variable TR_MaxAnalyzedSizeThreshold.
      if (analyzedSizeThreshold < maxAnalyzedSizeThreshold)
         analyzedSizeThreshold *= forceInlineMultiplier;

      sizeResetRequiredForForceInline = true;
      }
   ...
   for (TR_J9ByteCode bc = bci.first(); bc != J9BCunknown && bci._inlineableCallExists; bc = bci.next())
      ...
      if (_realSize > analyzedSizeThreshold / allowanceFactor)
         {
   ...
   if (sizeResetRequiredForForceInline)
      {
