...
void NullOperand::printToString(TR::StringBuf *buf)
{
    buf->appendf("(null)");
}
        ...
        case J9BCifacmpne: {
            ...
            if (lhsConstant && rhsConstant) {
                bool equal =
                    ...
                    && lhs->getKnownObjectIndex() == rhs->getKnownObjectIndex();
        ...
        if (fieldSymbol == NULL) {
            ...
            if (fail || avoidFolding) {
                ...
                if (tracer()->debugLevel()) {
                    ...
                    int32_t len = 0;
                    debugTrace(tracer(),
                        "dereference obj%d (%p), field +0x%x %s -> value %p: %s\n",
                        baseObjectIndex,
                        baseObjectAddress,
                        fieldOffset,
                        _calltarget->_calleeMethod->fieldName(cpIndex, len, this->trMemory()),
...
Operand *InterpreterEmulator::foldArrayLoad(
    Operand *obj, IconstOperand *indexOperand, TR::DataTypes type, TR_J9ByteCode bc)
{
#if defined(J9VM_OPT_JITSERVER)
    if (comp()->isOutOfProcessCompilation())
        return NULL;
    ...
    bool arrayClassOk = type == TR::Address
        ? TR::Compiler->cls.isReferenceArray(comp(), arrayClass)
    ...
    switch (type) {
        ...
        case TR::Int16: {
            ...
            int32_t value32 =
                bc == J9BCsaload ? (int32_t)value : (int32_t)(uint16_t)value;
...
void InterpreterEmulator::saveStack(int32_t targetIndex)
{
    ...
    TR_ASSERT_FATAL(
        _currentInlinedBlock->hasSuccessor(blocks(targetIndex)),
        "saving stack for edge %d -> %d which is not in CFG",
...
void InterpreterEmulator::setupBBStartLocalObjectState(int32_t index)
{
    ...
    if (localsAreUnknown) {
        ...
        heuristicTrace(tracer(),
            "block_%d at bc index %d has unvisited predecessor; conservative locals",
            _currentInlinedBlock->getNumber(),
            index);
    } else if (!_currentInlinedBlock->getExceptionPredecessors().empty()) {
        localsAreUnknown = true;
        heuristicTrace(tracer(),
            "block_%d at bc index %d is a catch block; conservative locals",
            _currentInlinedBlock->getNumber(),
            index);
    }
    ...
    if (index == 0
        && _currentInlinedBlock->getPredecessors().size() == 1
...
int32_t InterpreterEmulator::setupBBStartContext(int32_t index)
{
    ...
    if (_iteratorWithState && !_currentInlinedBlock->getExceptionPredecessors().empty()) {
        heuristicTrace(tracer(),
            "block_%d at bc index %d is a catch block; push unknown exception object",
            _currentInlinedBlock->getNumber(),
            index);
...
bool InterpreterEmulator::maintainStack(TR_J9ByteCode bc)
{
    ...
    switch (bc) {
        ...
        case J9BCgetfield:
            ...
            break;
        case J9BCarraylength: maintainStackForArraylength(); break;
        case J9BCaaload: maintainStackForArrayLoad(TR::Address, bc); break;
        case J9BClaload: maintainStackForArrayLoad(TR::Int64, bc); break;
        case J9BCiaload: maintainStackForArrayLoad(TR::Int32, bc); break;
        case J9BCsaload: // fall through
        case J9BCcaload: maintainStackForArrayLoad(TR::Int16, bc); break;
        case J9BCbaload: maintainStackForArrayLoad(TR::Int8, bc); break;
        case J9BCfaload: maintainStackForArrayLoad(TR::Float, bc); break;
        case J9BCdaload: maintainStackForArrayLoad(TR::Double, bc); break;
        case J9BCaload0:
            slotIndex = 0;
...
void InterpreterEmulator::maintainStackForReturn()
{
    assertHasState();
    TR_ASSERT_FATAL(
        _currentInlinedBlock->getSuccessors().size() == 1
        && _currentInlinedBlock->getSuccessors().front()->getTo() == _cfg->getEnd(),
        "block_%d returns and has incorrect successors",
        _currentInlinedBlock->getNumber());
...
void InterpreterEmulator::maintainStackForGetStatic()
{
    ...
    if (type == TR::Address
        && (canFold == TR_yes
            || (canFold == TR_maybe
                && TR::TransformUtil::enableEarlyGuardedStaticFinalFieldFolding()
        ...
        if (gotValue && value.isKnownObject()) {
            ...
            if (canFold == TR_maybe) {
                TR::Region &stackRegion = comp()->trMemory()->currentStackRegion();
                reqConst._assumptions.push_back(
                    new (stackRegion) TR::DeferredStaticFinalOSRAssumption(declaringClass));
            }
        }
    }
...
void InterpreterEmulator::maintainStackForInstanceof(int32_t cpIndex)
{
    ...
    if (obj != NULL) {
        ...
        if (castClass != NULL && objClass != NULL) {
            ...
            bool fixedCast = true;
            bool fixedObj =
                level == Operand::FIXED_CLASS || level == Operand::KNOWN_OBJECT;

            TR_YesNoMaybe isInstance =
                comp()->fe()->isInstanceOf(objClass, castClass, fixedObj, fixedCast);
...
bool InterpreterEmulator::shouldIterateWithState()
{
    ...
    if (argInfo != NULL) {
        TR_ASSERT_FATAL(
            argInfo->getNumArgs() == method()->numberOfParameters(),
            "wrong number of prex args %d, should be %d",
            argInfo->getNumArgs(),
        ...
        for (TR::ParameterSymbol *p = parms.getFirst(); p != NULL; p = parms.getNext()) {
            ...
            if (prexArg != NULL
                && TR_PrexArgument::knowledgeLevel(prexArg) == KNOWN_OBJECT) {
    ...
    for (; blockIt.currentBlock() != NULL; blockIt.stepForward()) {
        ...
        while (true) {
            ...
            if (_iteratorWithState) {
                ...
                if (tracer()->debugLevel()) {
                    debugTrace(tracer(),
                        "      operand stack after bytecode %+d : %s",
                        _bcIndex,
                        comp()->fej9()->getByteCodeName(nextByte(0)));
...
bool InterpreterEmulator::isEdgeUnreachable(TR::CFGEdge *edge)
{
    return _unreachableEdges.count(edge) != 0;
}

bool InterpreterEmulator::isBlockUnreachable(TR::Block *block)
{
    return _blockRc[block] == 0;
}
...
void InterpreterEmulator::markEdgeUnreachable(TR::CFGEdge *edge)
{
    ...
    debugTrace(tracer(),
        "%sedge %d (bci +%d) -> %d (bci +%d) is unreachable",
        dest->isCatchBlock() ? "exception " : "",
        src->getNumber(),
        src->getEntry()->getNode()->getByteCodeIndex(),
        dest->getNumber(),
