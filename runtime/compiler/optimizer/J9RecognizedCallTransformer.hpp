...
class RecognizedCallTransformer : public OMR::RecognizedCallTransformer
   ...
   /** \brief
    *     Transforms java/lang/MethodHandle.linkToVirtual or linkToInterface when the MemberName object (last arg) is not a known object.
    *
    *     linkToVirtual and linkToInterface are VM INL calls that would construct the call frame for the target method invocation.
    ...
    *     For directly-dispatched methods (vtable index 0), dispatchVirtual cannot be used as there are no vtable entries for those
    *     methods, so instead the call will be treated as a linkToSpecial call, as the target method can be obtained from the
    *     MemberName object.
    *
    *  \param treetop
    *     the TreeTop anchoring the call node
    *
    *  \param node
    *     the call node representing the linkToVirtual or linkToInterface call
    */
   void process_java_lang_invoke_MethodHandle_linkToVirtualInterface(TR::TreeTop * treetop, TR::Node * node);
#endif
